...
{   code *c;
       ...
       )
    {
        ...
        makeitextern(s);
        c = gencs(c,CALL,0,FLfunc,s);      // CALLF _trace
...

    {
        ...
        else if (xlocalsize == REGSIZE && (true || b->BC == BCret))
        {   mfuncreg &= ~mask[regx];
            c = gen1(c,0x58 + regx);                    // POP regx
        }
        else if (xlocalsize)
...
{ targ_size_t a;
  ...
  for (; c; c = code_next(c))
  {
    L1:
        ...
        {   int shortop = (c->Iflags & CFopsize) ? 1 : 0;
            int local_BPRM = BPRM;
