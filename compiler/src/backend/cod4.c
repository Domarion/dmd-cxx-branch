  ...
  {     int fl;
            ...
            !e1->Ecount)        /* and no CSE headaches */
        {
            ...
            if (e1->Eoper == OPind &&
                ...
                (true || e11->E1->EV.sp.Vsym->Sregm & IDXREGS)
               )
            {
...
{ regm_t retregs,forccs,forregs;
  ...
  if (wantres || (sz > REGSIZE && forccs))
  {
        ...
        {       regm_t idx;
                ...
                cs.Iop = 0x8B;
                NEWREG(cs.Irm,reg);
                cm = gen(nullptr,&cs);             // MOV reg,EA+2
                NEWREG(cs.Irm,findreglsw(retregs));
                getlvalue_lsw(&cs);
                cl = gen(nullptr,&cs);             // MOV reg+1,EA
                if (mask[reg] & idx)
                    ce = cat3(ce,cl,cm);
                else
                    ce = cat3(ce,cm,cl);
  ...
  switch (e2->Eoper)
  {
      ...
      case OPconst:
        ...
        {   // Just do a TEST instruction
            c = genregs(nullptr,0x85 ^ byte,reg,reg);      // TEST reg,reg
...
{ code *c,*ce,*c1,*c2,*c3,*c4;
        ...
        else
        {
            *pretregs &= ~mPSW;                 // flags are set by eval of e1
            ...
            else
            {
                ...
                c2 = getregs(retregs);
                reg = findreg(retregs);
                c3 = genregs(nullptr,0x89,reg,reg);    // MOV Ereg,Ereg
            }
        }
            ...
            I64 && op == OPs32_64)
  {
    ...
    {   code cs;
        ...
        if (!retregs)
            retregs = BYTEREGS;
        c1 = allocreg(&retregs,&reg,TYint);
        c2 = movregconst(nullptr,reg,0,0);                         //  XOR reg,reg
    ...
    L2:
        ...
        *pretregs &= ~mPSW;             /* flags are already set        */
        ...
        else
        {
            ...
            {   // AND reg,0xFFFF
                c3 = genc2(nullptr,0x81,modregrmx(3,4,reg),0xFFFFu);
            }
            else
            {
  ...
  else
  {
    ...
    c3 = genmovreg(nullptr,msreg,lsreg);                           // MOV msreg,lsreg
...
L2:
    c4 = fixresult(e,retregs,pretregs);
    return cat6(c0,c1,c2,c3,c4,nullptr);
}
...
{   code *c;
    ...
    return cat(c,fixresult(e,(mAX),pretregs));
}
...
code *cdbtst(elem *e, regm_t *pretregs)
{
    ...
    ty1 = tybasic(e1->Ety);
    word = (tysize[ty1] == SHORTSIZE) ? CFopsize : 0;
